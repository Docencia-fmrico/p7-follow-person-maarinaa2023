...
: Node("hsv_filter_node")
{
  ...
  camera_info_sub_ = create_subscription<sensor_msgs::msg::CameraInfo>(
    ...
    std::bind(&HSVFilter::camera_info_callback, this, _1));
...
void
HSVFilter::camera_info_callback(const sensor_msgs::msg::CameraInfo::ConstSharedPtr & info)
{
...
cv::Mat1b
HSVFilter::filter_image(cv::Mat & in, int h, int s, int v, int H, int S, int V)
{
...
void
HSVFilter::show_image_filtered(const cv::Mat & image, const cv::Mat1b & image_filtered)
{
  cv::waitKey(1);
...
cv::Point2d
HSVFilter::get_detected_center(const cv::Mat & filtered)
{
  ...
  const cv::Rect & bbx)
{
  if (detection_pub_->get_subscription_count() > 0) {
    ...
    detection_msg.header.frame_id = image->header.frame_id;  // WRONG!! image->header;
    detection_msg.header.stamp = image->header.stamp;

    vision_msgs::msg::Detection2DArray detection_array_msg;
    detection_array_msg.header.frame_id = image->header.frame_id;  // WRONG!! image->header;
    detection_array_msg.header.stamp = image->header.stamp;

    // Si no hay detección, enviamos valores en cero
    if (bbx.width == 0 || bbx.height == 0) {
      RCLCPP_WARN(get_logger(), "Publishing empty detection");
      detection_msg.bbox.center.position.x = 0.0;
      detection_msg.bbox.center.position.y = 0.0;
...
void
HSVFilter::image_callback(const sensor_msgs::msg::Image::ConstSharedPtr & image)
{
